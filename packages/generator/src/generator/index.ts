...
export const generateKitaRuntime = (
  ...
  ${providers.map((p) => toProviderImport(p, esm, src))}${toAllMethod(routes)}
  ${groupRouteImports(routes, esm, src)}
  ${groupRouteExports(routes, esm, src)}
...
export const toProviderImport = (provider: Provider, esm: boolean, src: string) => tst/* ts */ `
  import * as ${provider.type} from '${formatImport(provider.providerPath, esm, src)}'
`;
...
export const toRouteImport = (r: Route, esm: boolean, src: string) => tst/* ts */ `
  import * as ${r.controllerName} from '${formatImport(r.relativePath, esm, src)}'
`;

export const toRouteExport = (
  [path, names]: [path: string, names: string[]],
  esm: boolean,
  src: string
) => tst/* ts */ `
  export { ${names.join(', ')} } from '${formatImport(path, esm, src)}'
`;

// HELPERS

export function groupRouteImports(routes: Route[], esm: boolean, src: string) {
  return routes
    .filter((v, i, a) => a.findIndex((t) => t.relativePath === v.relativePath) === i)
    .map((f) => toRouteImport(f, esm, src));
}

export function groupRouteExports(routes: Route[], esm: boolean, src: string) {
  ...
  return Object.entries(imports).map((f) => toRouteExport(f, esm, src));
}
