...
if (require('piscina')?.isWorkerThread) {
  let AsyncController: Record<string, Function>;

  module.exports.asyncTest = async (args: any[]) => {
    // lazy loads AsyncController.
    if (!AsyncController) {
      AsyncController = require('./routes/async');
    }

    return AsyncController.get!.apply(undefined, args);
  };
